...
pub const DEFAULT_MTU: u32 = 450;
pub const DEFAULT_OBFUSCATION_KEY: u8 = 0xFA;
...
pub fn set_config(mtu: u32, key: Option<u8>) {
    let _ = CONFIG.set(SlipstreamConfig { mtu, key });
}

pub fn get_mtu() -> u32 {
    CONFIG.get().map(|c| c.mtu).unwrap_or(0)
}

pub fn get_obfuscation_key() -> Option<u8> {
    CONFIG.get().and_then(|c| c.key)
}
